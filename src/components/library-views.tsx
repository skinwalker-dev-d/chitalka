...
export function LibraryView({ greetingTemplate }: { greetingTemplate: string }) {
  ...
  return <>
    <div className="page-content">
      ...
      <div className="filter-row" aria-label="Фильтр по статусу">{filters.map((item) => <button key={item} onClick={() => setFilter(item)} className={filter === item ? "filter active" : "filter"}>{item}</button>)}<button className={`filter${(isGenreBrowserOpen || browsedGenre) ? " active" : ""}`} onClick={() => setIsGenreBrowserOpen(true)}><Sparkles size={12} /> Жанры</button></div>
    ...
    {isGenreBrowserOpen && <GenrePickerModal genres={allGenres} onClose={() => setIsGenreBrowserOpen(false)} onSelect={(genre) => { setIsGenreBrowserOpen(false); setBrowsedGenre(genre); }} />}
...
function GenrePickerModal({ genres, onClose, onSelect }: { genres: Array<{ name: string; count: number }>; onClose: () => void; onSelect: (genre: string) => void }) {
  return <div className="modal-backdrop" role="presentation" onMouseDown={onClose}><section className="book-composer genre-picker-modal" role="dialog" aria-modal="true" aria-labelledby="genre-picker-title" onMouseDown={(event) => event.stopPropagation()}><div className="composer-heading"><div><p className="eyebrow">Фильтр</p><h2 id="genre-picker-title">Жанры</h2></div><button type="button" className="icon-button" onClick={onClose} aria-label="Закрыть">×</button></div><div className="genre-list">{genres.map(({ name, count }) => <button type="button" key={name} className="genre-item" onClick={() => onSelect(name)}><span>{name}</span><small>{count} {count === 1 ? 'книга' : 'книг'}</small></button>)}</div>{!genres.length && <p className="empty-collection">Жанры пока не заданы.</p>}</section></div>;
}
...
export function CollectionsView() {
  ...
  function closeEditor() { setEditingCollection(null); setIsCreateOpen(false); setCollectionName(""); setCollectionColor(""); setSelectedBookIds([]); }
