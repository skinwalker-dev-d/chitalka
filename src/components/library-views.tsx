...
export function LibraryView({ greetingTemplate }: { greetingTemplate: string }) {
  ...
  function resetComposer() {
    ...
    setIsCollectionMenuOpen(false); setIsNewCollectionOpen(false); setNewCollectionName(""); setEditingBook(null);
  }
  ...
  return <>
    ...
    {isOpen && <div className={isBookFlying ? "modal-backdrop is-book-flying" : "modal-backdrop"} role="presentation" onMouseDown={closeBookComposer}>
      <form ref={composerRef} className={isBookFlying ? "book-composer is-book-flying" : "book-composer"} style={flightStyle ?? undefined} onSubmit={addNewBook} onMouseDown={(event) => event.stopPropagation()}>
        ...
        <label>Статус<select value={status} onChange={(event) => setStatus(event.target.value as BookStatus)} disabled={isBookFlying}>{filters.slice(1).map((item) => <option key={item}>{item}</option>)}</select></label>
...
export function CollectionsView() {
  ...
  function saveCollection(event: FormEvent<HTMLFormElement>) {
    ...
    if (editingCollection) updateCollection(editingCollection.id, collectionName, selectedBookIds);
    else createCollection(collectionName, selectedBookIds);
    closeEditor();
  }
