...
export function AppNavigation({ children }: { children: React.ReactNode }) {
  ...
  const avatarLightboxRef = useRef<HTMLDivElement>(null);
  function closeAvatar() {
    const el = avatarLightboxRef.current;
    if (!el || el.classList.contains("is-closing")) return;
    el.classList.add("is-closing");
    window.setTimeout(() => setIsAvatarOpen(false), 200);
  }
